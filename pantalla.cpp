...
void camaraAct(float* camaraPos, float x, float y, int widht, int height);
void salir();
...
void usarTeclado(ALLEGRO_DISPLAY* ventana)
{
	...
	while (!terminado)
	{
		...
		else
		{
			...
			camaraAct(camaraPos, x, y, ancho, alto);
			al_identity_transform(camara);
			al_translate_transform(camara, -camaraPos[0], -camaraPos[1]);
...
void camaraAct(float* camaraPos, float x, float y, int widht, int height)
{
	camaraPos[0] = -(ancho / 2) + (x + ancho / 2);
	camaraPos[1] = -(alto / 2) + (y + alto / 2);
	if (camaraPos[0] < 0)
	{
		camaraPos[0] = 0;
	}
